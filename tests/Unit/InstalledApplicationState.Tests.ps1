...
Describe 'Wintainium installed application state' {
    ...
    It 'reports structured validation errors for a missing application identifier' {
        $result = InModuleScope Wintainium.Core {
            Test-WintainiumInstalledApplicationState -State (New-TestInstalledApplicationState -Overrides @{ ApplicationId = '' })
        }
    ...
    It 'reports structured validation errors when a not-installed state contains a version' {
        $result = InModuleScope Wintainium.Core {
            Test-WintainiumInstalledApplicationState -State (New-TestInstalledApplicationState -Overrides @{ InstallationState = 'NotInstalled'; Version = '1.0' })
        }
    ...
    It 'accepts all defined architecture values' {
        foreach ($architecture in @('x86', 'x64', 'arm64', 'neutral', 'unknown')) {
            $result = InModuleScope Wintainium.Core -Parameters @{ Architecture = $architecture } {
                param($Architecture)
                Test-WintainiumInstalledApplicationState -State (New-TestInstalledApplicationState -Overrides @{ Architecture = $Architecture })
            }
    ...
    It 'accepts all defined channel values' {
        foreach ($channel in @('stable', 'prerelease', 'unknown')) {
            $result = InModuleScope Wintainium.Core -Parameters @{ Channel = $channel } {
                param($Channel)
                Test-WintainiumInstalledApplicationState -State (New-TestInstalledApplicationState -Overrides @{ Channel = $Channel })
            }
