    ...
    It 'does not persist or replace authoritative state when reconciliation evidence is Unknown' {
        InModuleScope Wintainium.Core {
            ...
            $result=[pscustomobject][ordered]@{
                IsSuccessful=$true; OperationId=$operationId; StageResults=@(
                    [pscustomobject]@{ StageName='ManifestValidation'; Execution=[pscustomobject]@{ Result=[pscustomobject]@{ Manifest=[pscustomobject]@{ Id='example.app' } } } }
                    ...
                    [pscustomobject]@{ StageName='Reconciliation'; Execution=[pscustomobject]@{ Result=[pscustomobject]@{ OperationId=$operationId; IsSuccessful=$true; Status='Reconciled'; Evidence=[pscustomobject]@{ ApplicationId='example.app'; InstallationState='Unknown'; EvidenceSource='Fixture' } } } }
                )
            ...
            try {
                $script:WintainiumOriginalApplicationUpdateLifecycle={ param($ManifestPath,$StateRoot,$MachineArchitecture,$DownloadRoot,$PluginRoot,$SchemaPath,$InstallerTimeoutMilliseconds,$HttpClient,$CancellationToken) $result }
            ...
            } finally { $script:WintainiumOriginalApplicationUpdateLifecycle=$original }
