...
Describe 'Wintainium public application update end-to-end regression' {
    It 'executes the complete lifecycle through the public command and returns the public projection' {
        InModuleScope Wintainium.Core {
            ...
            $result = Invoke-WintainiumApplicationUpdate -ManifestPath '/tmp/example.json' -StateRoot '/tmp/state' -MachineArchitecture x64 -DownloadRoot '/tmp/downloads'
    ...
    It 'executes the no-update path through the public command without entering download, verification, installation, or reconciliation' {
        InModuleScope Wintainium.Core {
            $operationId = [guid]::NewGuid().ToString()
            $manifest = [pscustomobject]@{
                Id='example.app'
            ...
            Mock Test-WintainiumApplicationDefinition { [pscustomobject]@{ OperationId=$operationId; IsValid=$true; Manifest=$manifest; ProviderPlugin=$provider; InstallerPlugin=$installer; ReconciliationPlugin=$reconciliation; Errors=@(); Warnings=@(); LogEvents=@() } }
            Mock Invoke-WintainiumProviderOperation { $release }
            Mock Get-WintainiumInstalledApplicationState { [pscustomobject]@{ ApplicationId='example.app'; InstallationState='Installed'; Version='2.0.0'; VersionSource='Fixture'; Architecture='x64'; Channel='stable'; InstallationLocation='/opt/example' } }
            Mock Get-WintainiumUpdateDecision { $decision }
            Mock Invoke-WintainiumDownload { throw 'download must not execute for a no-update lifecycle' }
            Mock Invoke-WintainiumArtifactVerification { throw 'verification must not execute for a no-update lifecycle' }
            Mock Invoke-WintainiumInstallerOperation { throw 'installer must not execute for a no-update lifecycle' }
            Mock Invoke-WintainiumReconciliationOperation { throw 'reconciliation must not execute for a no-update lifecycle' }
            Mock Invoke-WintainiumAuthoritativeStateReconciliation { throw 'persistence must not execute for a no-update lifecycle' }
            ...
            $result.OperationId | Should -Be $operationId
            $result.ApplicationId | Should -Be 'example.app'
            $result.Stages[-1].Status | Should -Be 'Skipped'
