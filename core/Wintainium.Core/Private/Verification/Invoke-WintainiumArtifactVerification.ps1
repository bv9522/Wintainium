function Invoke-WintainiumArtifactVerification {
    ...
    $resolvedOperationId = if ($PSBoundParameters.ContainsKey('OperationId')) { $OperationId } elseif ($DownloadResult.PSObject.Properties['OperationId']) { [string]$DownloadResult.OperationId } else { $null }
    $failed = {
        param([string]$Kind,[string]$Message)
        [pscustomobject][ordered]@{ OperationId=$resolvedOperationId; Status='Failed'; FailureKind=$Kind; Algorithm=$null; ExpectedHash=$null; ActualHash=$null; DestinationPath=if ($DownloadResult.PSObject.Properties['DestinationPath']) { [string]$DownloadResult.DestinationPath } else { $null }; ErrorMessage=$Message }
    }
    ...
    if (-not $DownloadResult.PSObject.Properties['DestinationPath'] -or [string]::IsNullOrWhiteSpace([string]$DownloadResult.DestinationPath)) { return & $failed 'DestinationMissing' 'The download result does not contain a destination path.' }

    $path = [string]$DownloadResult.DestinationPath
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { return & $failed 'DestinationMissing' 'The downloaded artifact does not exist as a regular file.' }
    ...
    try { $actualHash = (Get-FileHash -LiteralPath $path -Algorithm SHA256 -ErrorAction Stop).Hash.ToUpperInvariant() }
    ...
    foreach ($claim in $sha256Claims) {
        if ($actualHash -cne $claim.Value.ToUpperInvariant()) {
            return [pscustomobject][ordered]@{ OperationId=$resolvedOperationId; Status='Failed'; FailureKind='HashMismatch'; Algorithm='SHA256'; ExpectedHash=$claim.Value.ToUpperInvariant(); ActualHash=$actualHash; DestinationPath=$path; ErrorMessage='The downloaded artifact hash does not match the declared SHA256 verification evidence.' }
        }
    }

    [pscustomobject][ordered]@{ OperationId=$resolvedOperationId; Status='Verified'; FailureKind=$null; Algorithm='SHA256'; ExpectedHash=$sha256Claims[0].Value.ToUpperInvariant(); ActualHash=$actualHash; DestinationPath=$path; ErrorMessage=$null }
}
