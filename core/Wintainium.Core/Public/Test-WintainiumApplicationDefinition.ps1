function Test-WintainiumApplicationDefinition {
    ...
    $manifest = $null; $provider = $null; $installer = $null; $reconciliation = $null
    ...
    try {
        ...
        foreach ($errorRecord in @($manifestLoad.Errors)) { $errors.Add($errorRecord) }
    } catch { $errors.Add([pscustomobject][ordered]@{ Code='ManifestValidationFailed'; Path='$'; Message=$_.Exception.Message }) }
