function Get-WintainiumManifest {
    ...
    .PARAMETER Recurse
    Searches child directories beneath Path when specified.

    .PARAMETER SchemaPath
    Path to the application-manifest JSON schema used when importing manifests.

    .OUTPUTS
    PSCustomObject. The result contains OperationId, IsSuccessful, ManifestPaths,
    Manifests, Errors, Warnings, and LogEvents.

    .EXAMPLE
    Get-WintainiumManifest -Path 'C:\Wintainium\manifests'

    Discovers manifests directly in the specified collection directory.
