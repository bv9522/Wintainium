function Get-WintainiumApplicationRelease {
    ...
    .DESCRIPTION
    This command is the Core boundary between a validated application definition and
    provider-backed release discovery. Manifest validation and plugin resolution occur
    before the provider is invoked. The provider receives only a purpose-built discovery
    request and its normalized result is returned through the Core operation boundary.
    No download or installation work is performed.

    .PARAMETER ManifestPath
    Path to the application manifest whose upstream releases should be discovered.
    ...
    Validates the application definition and discovers its releases through the
    provider named by the manifest.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
